...
Object queryForObject(String sql, RowMapper rm)
	: 인파라미터가 없고, 하나의 레코드나 결과값을 반환하는 select계열의 쿼리문을 실행할 때 사용한다.
	Ex)	아이디 / 하나의 게시물 / 카운트
Object queryForObject(String sql, Object[] args, RowMapper rm)
	: 인파라미터가 있고, 하나의 레코드를 반환하는 select게열의 쿼리문 실행에 사용한다.
 
List query(String sql, RowMapper rm)
	: 인파라미터가 없는 여러개의 레코드를 반환하는 select계열의 쿼리문인 경우 사용한다.
...
public class JDBCTemplateDAO {
	...
	public JDBCTemplateDAO() {
		/*
			...
			정적변수인 JdbcTemplateConst.template에 값을 할당하였으므로,
