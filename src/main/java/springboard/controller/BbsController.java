...
/*
	기본패키지로 설정한 곳에 컨트롤러를 선언하면 요청이 들어왔을때 Auto scan된다.
...
public class BbsController {
	/*
	@Autowired
		: 스프링 설정파일(servlet-context.xml)에서 이미 생성된 빈을
		  자동으로 주입받고 싶을때 사용한다.
		  타입을 기반으로 자동주입되며, 만약 해당 타입의 빈이 존재하지 않으면
		  에러가 발생되어 서버를 시작할 수 없다.
		  
	 - 생성자, 멤버변수, 메소드(setter)에 적용할 수 있다.
	 - 타입을 이용해 자동으로 프로퍼티 값을 설정한다.
	 - 해당 어노테이션은 멤버변수에만 적용할 수 있다.
	   메소드 내의 지역변수에는 사용할 수 없다.
	 - 타입을 통해 자동으로 설정되므로 같은 타입이 2개이상 존재하면 예외가 발생한다.
	 	; 빈과 반드시 1대1 대응이 되어야 한다.
	 - 프로그램이 처음 구동될 때 @Autowired가 걸려있는 요소들을 제일먼저 실행한다.
	 	(static과 비슷한 느낌)
	 */
	// 빈을 주입받기 위한 변수 선언
	...
	/*
		...
		해당 빈은 스프링 컨테이너가 시작될때 생성되며,
		타입을 기반으로 자동 주입 받게된다.
	 */
	@Autowired
	// 연결이 되었는지 확인하기 위한 setter()
